...
% TODO: Input layer, hidden layers and output layer

\subsection{Linearity}

\subsubsection{Linear perceptron}

Linear perceptrons classifiers, also called \textbf{single-layer perceptrons}, are the \textit{simplest} feedfoward neural network available.
...
The two main activation functions are the \textbf{sigmoid} and the \textbf{ReLU} function as shown in \autoref{fig:activation-functions}.
...
The equation for the \textit{sigmoid} function is $s(x)=\frac{1}{1 + e^{-x}}$, which maps its values to the range $\left[0, 1\right]$. \\
The \textit{ReLU} is expressed as $r(x)=max(0, x)$ and it ignores everything but the positive part of its argument.
